...
#include "../Runtime/missing_runtime_stub.h"
...
// we need to work out suitable types for this ref count implementation
// 16-bit out of line
template <>
struct RefCountBitsInt<RefCountNotInline, 2> {
  typedef uint32_t Type;
  typedef int32_t SignedType;  
};
...
// 16-bit inline
template <>
struct RefCountBitsInt<RefCountIsInline, 2> {
...
// 16-bit inline
template <>
struct RefCountBitOffsets<2> {
  static const size_t IsImmortalShift = 0;
  static const size_t IsImmortalBitCount = 1;
  static const uint16_t IsImmortalMask = maskForField(IsImmortal);
  
  static const size_t UnownedRefCountShift = shiftAfterField(IsImmortal);
  static const size_t UnownedRefCountBitCount = 6;
  static const uint16_t UnownedRefCountMask = maskForField(UnownedRefCount);

  static const size_t IsDeinitingShift = shiftAfterField(UnownedRefCount);
  static const size_t IsDeinitingBitCount = 1;
  static const uint16_t IsDeinitingMask = maskForField(IsDeiniting);
  
  static const size_t StrongExtraRefCountShift = shiftAfterField(IsDeiniting);
  static const size_t StrongExtraRefCountBitCount = 7;
  static const uint16_t StrongExtraRefCountMask = maskForField(StrongExtraRefCount);
  
  static const size_t UseSlowRCShift = shiftAfterField(StrongExtraRefCount);
  static const size_t UseSlowRCBitCount = 1;
  static const uint16_t UseSlowRCMask = maskForField(UseSlowRC);

  static const size_t SideTableShift = 0;
  static const size_t SideTableBitCount = 14;
  static const uint16_t SideTableMask = maskForField(SideTable);
  static const size_t SideTableUnusedLowBits = 2;

  static const size_t SideTableMarkShift = SideTableBitCount;
  static const size_t SideTableMarkBitCount = 1;
  static const uint16_t SideTableMarkMask = maskForField(SideTableMark);
};
...
class RefCounts {
  ...
  bool doDecrementNonAtomicSlow(RefCountBits oldbits, uint32_t dec) {
    ...
    else if (oldbits.isImmortal(true)) {
      return false;
    }
    else if (oldbits.hasSideTable()) {
      // Decrement failed because we're on some other slow path.
...
// _Static_assert(_Alignof(InlineRefCounts) == 1,"1");
// _Static_assert(_Alignof(InlineRefCounts) == 2,"2");

// _Static_assert(_Alignof(__swift_uintptr_t) == 1,":1");
// _Static_assert(_Alignof(__swift_uintptr_t) == 2,":2");

// _Static_assert(_Alignof(swift::RefCountBits) == 1,"::1");
// _Static_assert(_Alignof(swift::RefCountBits) == 2,"::2");
